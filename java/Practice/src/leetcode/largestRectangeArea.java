    ...
    public int maximalRectangle(char[][] matrix) {
        ...
        for (int i = 0; i < row; i++) {
            ...
            for (int j = 0; j < col; j++) {
                ...
                while (tmp.isEmpty() || tmp.peek() != j) {
                    if (tmp.isEmpty() || nums[j] > nums[tmp.peek()]) {
                        tmp.push(j);
                    } else {
                        int index = tmp.pop();
                        int num = nums[index];

                        maxArea = Math.max(maxArea, (tmp.isEmpty() ? j : j - tmp.peek() - 1) * num);
                    }
                }
            ...
            while(!tmp.isEmpty()){
                int index = tmp.pop();
                int num = nums[index];
                maxArea = Math.max(maxArea, num*(tmp.isEmpty()? index +1  : index - tmp.peek()));         
            }
        }
